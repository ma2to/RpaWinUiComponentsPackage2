...
using RpaWinUiComponentsPackage.AdvancedWinUiDataGrid.Models.Core;
...
public class DynamicTableCore
{
        ...
        Microsoft.Extensions.Logging.ILogger? logger = null)
    {
        try
        {
            _logger.Info("🚀 OPERATION START: DynamicTableCore.InitializeAsync - Columns: {Count}, EmptyRows: {EmptyRows}", 
                                   columns.Count, emptyRowsCount);
            ...
            _logger?.Info("✅ OPERATION SUCCESS: DynamicTableCore initialized - Columns: {ColumnCount}, Rows: {RowCount}", 
                                   _columns.Count, _rows.Count);
        }
        catch (Exception ex)
        {
            _logger?.Error(ex, "🚨 INIT ERROR: DynamicTableCore initialization failed");
            throw;
        }
    }
        ...
        IProgress<ExportProgress>? exportProgress = null)
    {
        ...
        try
        {
            _logger?.Info("🚀 OPERATION START: ExportToDictionaryAsync - Rows: {Count}, IncludeValidation: {IncludeValidation}", 
                                   _rows.Count, includeValidationAlerts);
            ...
            _logger?.Info("✅ OPERATION SUCCESS: ExportToDictionaryAsync - Exported: {Rows} rows, Time: {ElapsedMs}ms", 
